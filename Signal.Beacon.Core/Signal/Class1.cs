...
namespace Signal.Beacon.Core.Signal
{
    ...
    public class Auth0DeviceAuthorization
    {
        ...
        {
            using var response = await new HttpClient().PostAsync(DeviceCodeUrl, new FormUrlEncodedContent(new List<KeyValuePair<string?, string?>>
            {
        ...
        public async Task<AuthToken?> WaitTokenAsync(DeviceCodeResponse codeResponse, CancellationToken cancellationToken)
        {
